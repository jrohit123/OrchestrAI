-- ═══════════════════════════════════════════════════════════════════════════
-- MIGRATION 011 — generic constraint "gates" for workflows (GODREJ EMERALD)
--
-- Identical schema change to migrations/011_baanganga_gates_schema.sql —
-- baanganga and godrej_emerald share the same workflows/workflow_drafts/
-- pending_approvals/roles table shape for this part of the schema (they only
-- diverge on business tables: invoices/customers/inventory vs
-- cases/residents/staff/committee_*). Kept as a separate file, as requested,
-- so each database gets its own migration to run directly.
--
...
--    "when":{"field":"$computed.total_amount","gte":10000},
--    "levels":[
--      {"level":1,"role":"treasurer","max_amount":null}
--    ]}
--
--   {"id":"appr_lift","type":"approval_chain",
--    "when":{"field":"$fields.category","equals":"lift_elevator"},
--    "levels":[
--      {"level":1,"role":"secretary","max_amount":null}
--    ]}
--
--   {"id":"perm1","type":"permission","role_any_of":["committee"]}
--
...
--
-- pending_approvals gets gate_id/level so a multi-level chain (e.g. committee
-- treasurer, then secretary) can track which stage is currently pending,
-- instead of a single implicit one-shot approval.
--
...
--
-- Org: Godrej Emerald (data_sources.source_key = 'godrej_emerald')
-- Run this against the Godrej Emerald database directly.
-- ═══════════════════════════════════════════════════════════════════════════
BEGIN;
...
-- change: the same role that could approve before still can. (Godrej's
-- committee-driven roles, e.g. 'treasurer'/'secretary', are unaffected by
-- this backfill since no existing workflow here used approval_threshold
-- against those roles specifically — legacy rows fall back to 'owner'.)
