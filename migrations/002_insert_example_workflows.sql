...
INSERT INTO workflows (
    ...
    '{"metal_type": {"table": "pricing", "column": "metal_type", "match": "ILIKE", "required": false, "format": "wildcard", "values": ["22kt", "18kt", "24kt", "silver", "platinum", "gold"]}}'::jsonb,
    'SELECT metal_type, rate_per_gram, making_charge_pct, updated_at FROM pricing WHERE org_id = $1 AND quotation_number IS NULL ORDER BY metal_type',
    '[]'::jsonb,
    'pricing',
    '{"bhav": "rate", "sone ka bhav": "gold rate", "aaj ka": "current"}'::jsonb,
    'This workflow checks current metal rates. Queries pricing table for rate entries (quotation_number IS NULL), shows rate per gram and making charge. Glossary: bhav=rate, sone ka bhav=gold rate. Examples: "gold rate" → show all rates, "22kt rate kya hai" → filter by 22kt, "aaj ka bhav" → show all rates. No entity filtering in SQL - returns all rates.',
    '[]'::jsonb,
    NULL,
    false,
    NULL,
