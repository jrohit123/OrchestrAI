...
def build_draft_recap(draft: dict) -> str:
    ...
    Numbered, one-fact-per-line layout (Title / Type / Fields / Constraints /
    Who can use it / Trigger command) — a wall of unlabeled text ("Type:
    action\\n\\nConstraints: none\\n\\nWho can use it: ...") read as one
    blob to admins; numbering makes it scannable at a glance.
    """
    lines = []
    n = 1
    ...
    if gates:
        lines.append(f"{n}) Constraints:")
        for g in gates:
            lines.append(f"   {_describe_gate(g)}")
    else:
        lines.append(f"{n}) Constraints: none")
    n += 1
...
def _derive_slash_command(intent_key: str) -> str:
    ...
    stripped (e.g. intent_key "assign_case" -> "/assigncase") so an admin can
    tell at a glance which command maps to which workflow. Enforced here —
    deterministically, from the compiler's own intent_key — rather than left
    to whatever the admin and LLM negotiated in chat, so it can never drift.
    """
    return re.sub(r"[^a-z0-9]", "", (intent_key or "").lower())[:32] or "workflow"


def _describe_step(step: dict) -> str:
    """One line of plain English for a single workflows.steps[] entry."""
    if not isinstance(step, dict):
        return str(step)
    op = step.get("op", "")
    p = step.get("params") or {}
    table = p.get("table", "the record")
    if op == "resolve_entity":
        return f"Look up the {table} matching what the user gave"
    if op == "compute":
        return "Run the calculations listed above"
    if op == "otp_gate":
        return "If the amount is over the threshold, ask for OTP verification"
    if op == "approval_gate":
        return "If the amount is over the threshold, ask for manager approval"
    if op == "db.insert_row":
        return f'Save a new row in "{table}"'
    if op == "db.update_row":
        return f'Update the matching row in "{table}"'
    if op == "db.upsert_row":
        return f'Save or update the row in "{table}"'
    if op == "pdf.generate":
        return "Generate a PDF document"
    if op == "notify.whatsapp":
        return "Send the result to the user on WhatsApp"
    return f"({op or 'unrecognized step'})"


def describe_workflow_logic(w: dict) -> str:
    """
    Deterministic, plain-English rendering of a PUBLISHED workflow's full
    logic — fields collected, automatic calculations, constraints, and the
    step-by-step pipeline. This is what "View raw JSON" should show a
    non-technical admin by default; generated straight from the saved row
    (same principle as build_draft_recap), not from an LLM's account of it.
    Raw JSON stays available underneath for anyone who actually needs it.
    """
    entity_schema = _parse_jsonb(w.get("entity_schema"), {}) or {}
    calc_rules    = _parse_jsonb(w.get("calc_rules"), {}) or {}
    steps         = _parse_jsonb(w.get("steps"), []) or []
    gates         = _parse_jsonb(w.get("gates"), []) or []

    lines = []
    n = 1

    # ── What it collects ─────────────────────────────────────────────
    lines.append(f"{n}) What it collects from the user"); n += 1
    items_schema = None
    field_lines = []
    for name, fs in entity_schema.items():
        if not isinstance(fs, dict):
            continue
        if name == "items" and fs.get("type") == "array":
            items_schema = fs.get("item_schema") or {}
            continue
        if fs.get("computed"):
            continue
        req = "required" if fs.get("required") else "optional"
        field_lines.append(f"   • {name} ({req}, {fs.get('type', 'text')})")
    lines.extend(field_lines or ["   (nothing — this workflow takes no input)"])
    if items_schema:
        lines.append("   • A list of items, each with:")
        for iname, ifs in items_schema.items():
            if not isinstance(ifs, dict) or ifs.get("computed"):
                continue
            req = "required" if ifs.get("required") else "optional"
            lines.append(f"      - {iname} ({req}, {ifs.get('type', 'text')})")
    lines.append("")

    # ── What's calculated automatically ─────────────────────────────
    lines.append(f"{n}) Calculated automatically"); n += 1
    item_rules      = calc_rules.get("item_rules") or {}
    aggregate_rules = calc_rules.get("aggregate_rules") or {}
    if item_rules or aggregate_rules:
        for field, formula in item_rules.items():
            lines.append(f"   • {field} (per item) = {formula}")
        for field, formula in aggregate_rules.items():
            lines.append(f"   • {field} = {formula}")
    else:
        lines.append("   nothing — no automatic calculations")
    lines.append("")

    # ── Constraints ──────────────────────────────────────────────────
    lines.append(f"{n}) Constraints"); n += 1
    if gates:
        for g in gates:
            lines.append(f"   {_describe_gate(g)}")
    else:
    ...
    # ── Step-by-step ─────────────────────────────────────────────────
    lines.append(f"{n}) What happens when it runs"); n += 1
    if steps:
    ...
    elif w.get("sql_template"):
        table_match = re.search(r"\bFROM\s+(\w+)", w["sql_template"], re.IGNORECASE)
        table = table_match.group(1) if table_match else "the database"
        lines.append(f'   Looks up matching rows from "{table}" and shows them to the user')
    else:
