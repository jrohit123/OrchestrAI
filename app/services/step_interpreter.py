...
def _validate_identifier(name: str, identifier_type: str = "identifier") -> None:
    ...
    if not IDENTIFIER_PATTERN.match(name):
        raise StepError(
            f"Invalid {identifier_type}: '{name}'. Must match ^[a-z_][a-z0-9_]*$"
        )
...
async def _normalize_enum_values(table: str, values: dict, source_key: str) -> None:
    ...
    enum (see schema_utils.get_enum_constraints), snap a loosely-matching
    string (case/space/hyphen-insensitive) to the DB's canonical value. This
    is the deterministic LAST line of defense — entity_schema now carries
    each field's real enum options (see workflow_compiler.txt) so the
    runtime chat agent captures the correct canonical value using actual
    language understanding in the first place; this catches whatever still
    slips through that. If the column is enum-constrained but nothing
    ...
    """
    from app.services.schema_utils import get_enum_constraints

    enums = await get_enum_constraints(source_key)
    if not enums:
        return
    for col, val in list(values.items()):
        ...
        match = next(
            (
                a
                for a in allowed
                if re.sub(r"[\s-]+", "_", a.strip().lower()) == normalized
            ),
            None,
        )
        if match:
...
def _eval_when_condition(cond: dict, ctx: dict) -> bool:
    ...
    if "in" in cond:
        return str(actual).lower() in [
            str(_when_value(v, ctx)).lower() for v in cond["in"]
        ]
    if "not_in" in cond:
        return str(actual).lower() not in [
            str(_when_value(v, ctx)).lower() for v in cond["not_in"]
        ]
    if "exists" in cond:
...
async def _op_resolve_entity(params: dict, ctx: dict) -> dict:
    ...
    if match_columns:
        ...
        for col, path in match_columns.items():
            ...
            if val in (None, ""):
                raise StepError(
                    f"resolve_entity: no value at '{path}' for match_columns.{col}"
                )
            safe_val = (
                str(val).replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")
            )
    ...
    name_val = str(
        name_val
    )  # NEW — handles UUID/int/etc. values (e.g. $case.complainant_id) safely
    ...
    if (
        ...
        if not row:
            raise StepError(
                "Could not resolve your own user record for self-assignment"
            )
        resolved = dict(row)
        ctx[into] = resolved
        ctx.setdefault("_resolved_entities", []).append(into)
            ...
            if not raw_rows:
                ...
                if m:
                    ...
                    for row in candidates:
                        ...
                        if (
                            cm
                            and cm.group(1) == needle_prefix
                            and int(cm.group(2)) == int(needle_digits)
                        ):
                            matched.append(row)
                    raw_rows = matched[:5]
            rows = [dict(r) for r in raw_rows]
...
async def _op_compute(params: dict, ctx: dict) -> dict:
    ...
    verified = await verify_draft(
        ctx["workflow"], ctx["fields"], ctx["org_id"], ctx["source_key"]
    )
    ctx["fields"] = verified
...
async def _op_otp_gate(params: dict, ctx: dict) -> dict:
    ...
    if otp_gates:
        ...
        otp_threshold = float(
            _parse_jsonb(ctx["workflow"].get("otp_threshold"), 0) or 0
        )
...
async def _op_approval_gate(params: dict, ctx: dict) -> dict:
    ...
    if approval_gates:
        ...
        approval_threshold = float(
            _parse_jsonb(ctx["workflow"].get("approval_threshold"), 0) or 0
        )
        if approval_threshold > 0 and amount >= approval_threshold:
            queue.append(
                {"gate_id": "legacy", "level": 1, "role": None, "max_amount": None}
            )
    ...
    approver_role_name = first["role"] or (
        approver.get("role_name") if approver else "approver"
    )
    ...
    if approver:
        ...
        level_note = (
            f" — level {first['level']} of {len(queue)}" if len(queue) > 1 else ""
        )
        await _send_buttons(
            to=approver["phone"],
            body=(
...
async def _op_insert_row(params: dict, ctx: dict) -> dict:
    ...
    if table.startswith("sheet:"):
        from app.services.sheets_client import sheet_count_rows, sheet_insert_row
    ...
    try:
        ...
        if not row:
            raise StepError(
                f"db.insert_row failed: INSERT returned no rows for table '{table}'"
            )
        ...
        raise StepError(f"Failed to insert into {table}: {e!s}")
...
async def _op_generate_pdf(params: dict, ctx: dict) -> dict:
    ...
    try:
        subtitle = (
            subtitle_tmpl.format(**all_ctx)
            if subtitle_tmpl
            else params.get("subtitle", "")
        )
    except (KeyError, IndexError):
...
async def _op_conflict_check(params: dict, ctx: dict) -> dict:
    ...
    error_message = (
        params.get("error_message") or "This conflicts with an existing record."
    )
    ...
    for col, path in match_columns.items():
        ...
        if val is None:
            raise StepError(
                f"conflict_check: no value at '{path}' for match_columns.{col}"
            )
...
async def run_workflow_steps(
    ...
    for gate in [
        g for g in gates if isinstance(g, dict) and g.get("type") == "permission"
    ]:
        allowed_roles = set(gate.get("role_any_of") or [])
        if allowed_roles and user.get("role") not in allowed_roles:
            return {
                "status": "error",
                "message": gate.get("denied_message")
                or (
    ...
    try:
        for i in range(resume_step, len(steps)):
            ...
            try:
                ...
                if op_name in ("pdf.generate", "notify.whatsapp") and ctx.get(
                    "inserted"
                ):
                    logger.warning(
                        f"Non-fatal failure in '{op_name}' after successful insert: {e}"
                    )
    ...
    if ctx.get("pdf_bytes") and "notify.whatsapp" not in [
        s.get("op") if isinstance(s, dict) else json.loads(s).get("op") for s in steps
    ]:
